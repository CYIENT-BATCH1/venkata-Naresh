...
unsigned char G_Eng_EngineTemperature_uchar = 50u ;
for(int i=7;i>=0;i--){
printf("%x ",(G_Eng_EngineTemperature_uchar>>i)&1);  // conveted the decimal into binary  (8 bit)
}
unsigned int G_Eng_EngineRpm_uint = 1000u;
printf("\n");
for(int j=12;j>=0;j--){
printf("%x ",(G_Eng_EngineRpm_uint>>j)&1);    //conveted the decimal into binary  (12 bit)
}
